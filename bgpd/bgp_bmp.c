...
static void bmp_queue_entry_clear(struct bmp_queue_entry *bqe)
{
	if (!bqe)
		return;

	if (bqe->attr)
		bgp_attr_unintern(&bqe->attr);
...
static struct attr *bmp_attr_ref(struct attr *attr)
{
	...
	/* bgp_attr_unintern() releases the attr's referenced subobjects even
	 * when the attr itself remains alive, so a retained attr reference must
	 * be acquired through bgp_attr_intern() to bump those subobject refs too.
	 */
		  ...
		  bool post_policy)
{
	...
	if (stream_get_endp(s) > BGP_MAX_PACKET_SIZE) {
		zlog_warn("bmp: skipping adj-rib-out route-monitoring for %pFX: encoded UPDATE exceeds %u bytes",
			  p, BGP_MAX_PACKET_SIZE);
		stream_free(s);
		return NULL;
	}
		     ...
		     safi_t safi, size_t refcount)
{
	...
	frr_each_safe (bmp_qlist, &bt->outprelist, bqe) {
		if (!bmp_adjout_pre_entry_matches(bqe, peerid, afi, safi))
			continue;

		bmp_qlist_del(&bt->outprelist, bqe);
...
static bool bmp_wrqueue_adjout(struct bmp *bmp, struct pullwr *pullwr)
{
	...
	afi_t afi = bqe->afi;
	safi_t safi = bqe->safi;
	uint8_t mon_flag = bqe->out_post_policy ? BMP_MON_OUT_POSTPOLICY
...
static void bmp_wrfill(struct bmp *bmp, struct pullwr *pullwr)
{
	...
	case BMP_Run:
		...
		if (bmp_wrqueue(bmp, pullwr))
			break;
		if (bmp_wrqueue_adjout(bmp, pullwr))
			break;
...
static void bmp_queue_entry_update_attr(struct bmp_queue_entry *bqe,
					struct attr *attr,
...
static void bmp_outpre_update(struct bmp_targets *bt,
			      ...
			      size_t refcount)
{
	...
	bqe = XCALLOC(MTYPE_BMP_QUEUE, sizeof(*bqe));
	memcpy(bqe, bqeref, sizeof(*bqe));
	bmp_queue_entry_update_attr(bqe, attr, labels);
	bmp_qlist_add_tail(&bt->outprelist, bqe);
}
		       ...
		       uint32_t addpath_tx_id, bool post_policy)
{
	...
	if (!post_policy) {
		bmp_outpre_update(bt, &bqeref, attr, labels, refcount);
		return NULL;
	}
...
static int bmp_process_adjout(struct update_subgroup *subgrp,
			      ...
			      bool withdraw)
{
	...
	struct listnode *node;
	struct peer_af *paf;
	struct peer *from = path ? path->peer : NULL;
	struct bmp_queue_entry *last_item;
	...
	for (ALL_LIST_ELEMENTS_RO(bm->bgp, node, bgp_vrf)) {
		...
		frr_each (bmp_targets, &bmpbgp->targets, bt) {
			if (!CHECK_FLAG(bt->afimon[afi][safi], mon_flag))
				continue;
			if (bgp_vrf != bgp &&
			    !bmp_imported_bgp_find(bt, bgp->name))
				continue;
			...
			SUBGRP_FOREACH_PEER (subgrp, paf) {
				...
				last_item = bmp_process_one_adjout(
					bt, afi, safi, bn, peer, from,
					withdraw ? NULL : attr,
					withdraw ? NULL : labels, addpath_tx_id,
					post_policy);
				if (!last_item)
					continue;
...
static void bmp_targets_put(struct bmp_targets *bt)
{
	...
	while ((bqe = bmp_qlist_pop(&bt->outprelist)))
		bmp_queue_entry_free(bqe);
	bmp_qlist_fini(&bt->outprelist);
	  ...
	  "Send outbound state with policy and outbound filtering applied\n")
{
	...
	else if (BMP_POLICY_IS_OUT(policy)) {
		/* Case of Adj-RIB-out */
